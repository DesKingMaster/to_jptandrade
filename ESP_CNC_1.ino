...
ESP8266WebServer server(7129);   //instantiate server at port 80 (http port) //2222 for wemos //
...
void setup(void){
  ...
  server.on("/data_update.js", [](){
    //Posvalues = (String)stepper1.currentPosition() + "  " + (String)stepper2.currentPosition() + "  " +(String)stepper3.currentPosition() + "  " + (String)stepper4.currentPosition() + "  " + (String)stepper5.currentPosition();
  ...
  server.on("/data.txt", [](){
    server.send(200,"text/html", Posvalues);
  });
